...
export default function MenuSection({ menuItems, categories, initialCategory }) {
    const [openCategory, setOpenCategory] = useState(initialCategory || categories?.[0]?.id || null);
    const { cart, addToCart, updateQty } = useCart();
    const { deliveryAvailable } = useLocation();

    const canOrder = deliveryAvailable !== false;
                            ...
                            <div className="restaurant-card-info" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                                    ...
                                    {!canOrder ? (
                                        ...
                                        qty > 0 ? (
                                            /* Quantity counter — shows when item is in cart */
                                            <div className="qty-counter" id={`qty-counter-${item.id}`}>
                                                <button
                                            ...
                                            <button onClick={() => addToCart(item)} className="add-to-cart-btn" id={`add-cart-${item.id}`} style={{
