...
export async function GET() {
    try { return NextResponse.json(await getCollections()); }
...
export async function POST(request) {
    ...
    try {
        ...
        const data = { id: 'col-' + Date.now(), title: b.title || '', image: b.image || '', link: b.link || '/menu' };
...
export async function PUT(request) {
    ...
    try {
        const b = await request.json();
        const { id, ...rest } = b;
        await updateCollection(id, rest);
        return NextResponse.json({ success: true });
    } catch (e) { return NextResponse.json({ error: e.message }, { status: 500 }); }
}
export async function DELETE(request) {
    const d = await checkAdmin(); if (d) return d;
    try { const { searchParams } = new URL(request.url); await deleteCollection(searchParams.get('id')); return NextResponse.json({ success: true }); }
    catch (e) { return NextResponse.json({ error: e.message }, { status: 500 }); }
}
