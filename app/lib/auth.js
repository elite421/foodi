...
export async function sendOtp(phone) {
    ...
    console.log(`${code} otp sent to ${phone}`);
    ...
    // Check if SMS API keys are configured
    const msgClubKey = process.env.MSGCLUB_AUTH_KEY;
    const fast2SmsKey = process.env.FAST2SMS_API_KEY;

    if (msgClubKey) {
        try {
            // MSGCLUB API integration
            const message = encodeURIComponent(`Your FooodieClub login OTP is: ${code}. Valid for 10 minutes.`);
            const senderId = process.env.MSGCLUB_SENDER_ID || "DEMOOS";
            const url = `http://msg.msgclub.net/rest/services/sendSMS/sendGroupSms?AUTH_KEY=${msgClubKey}&message=${message}&senderId=${senderId}&routeId=1&mobileNos=${phone}&smsContentType=english&entityid=NoneedIfAddedInPanel&tmid=140200000022&templateid=NoneedIfAddedInPanel&concentFailoverId=30`;

            const response = await fetch(url, { method: 'GET' });

            if (!response.ok) {
                const text = await response.text();
                console.error('MSGCLUB Error:', text);
                throw new Error(text || 'Failed to send OTP via MSGCLUB');
            }
            console.log(`[SMS SENT] OTP sent to ${phone} successfully via MSGCLUB`);

        } catch (error) {
            console.error('Failed to send OTP via MSGCLUB:', error);
        ...
        try {
            ...
            const response = await fetch('https://www.fast2sms.com/dev/bulkV2', {
                ...
                body: JSON.stringify({
                    ...
                    message: `Your FooodieClub login OTP is: ${code}. Valid for 10 minutes.`,
                    language: 'english',
                    flash: 0,
                    numbers: phone
                })
            ...
            if (!data.return) {
                ...
                throw new Error(data.message || 'Failed to send OTP via SMS');
            }
            console.log(`[SMS SENT] OTP sent to ${phone} successfully via Fast2SMS`);

        } catch (error) {
            console.error('Failed to send OTP via Fast2SMS:', error);
            throw new Error(error.message || 'Failed to send OTP via SMS');
        }
    } else {
        // Fallback for development if no key is present
        console.warn(`[WARNING] No SMS API keys are set. OTP for ${phone}: ${code} (not actually sent)`);
    }
