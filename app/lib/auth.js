...
export async function sendOtp(phone) {
    ...
    console.log('[AUTH DEBUG] MSG91_AUTH_KEY from env:', msg91Key ? 'EXISTS (length: ' + msg91Key.length + ')' : 'NOT FOUND');
    console.log('[AUTH DEBUG] All env vars starting with MSG:', Object.keys(process.env).filter(k => k.startsWith('MSG')));

    if (msg91Key) {
        try {
            // Format phone number with country code for MSG91
            const phoneWithCountryCode = phone.startsWith('+') ? phone : `+91${phone}`;
            
            console.log('[AUTH DEBUG] Calling sendOTP with:', phoneWithCountryCode, code);
            
            // Use MSG91 to send OTP
            const result = await sendOTP(phoneWithCountryCode, code);
            
            console.log('[AUTH DEBUG] sendOTP result:', result);
            
            if (result.success) {
                console.log(`[SMS SENT] OTP sent to ${phoneWithCountryCode} successfully via MSG91`);
            } else {
