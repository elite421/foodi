...
export async function getAllSiteData() {
  const [hero, categories, menuItems, offers, collections, elite, eliteItems, settings, coupons, brands] = await Promise.all([
    ...
    prisma.brand ? prisma.brand.findMany({ where: { isActive: true } }) : Promise.resolve([])
  ]);
  ...
  return {
    ...
    collections: collections.map(c => ({ id: c.id, title: c.title, image: c.image, link: `/collection/${c.id}`, description: c.description, menuItemIds: c.menuItemIds })),
    coupons: coupons.map(c => ({ id: c.id, code: c.code, discountType: c.discountType, discountValue: c.discountValue })),
    brands: brands.map(b => ({ id: b.id, name: b.name, slug: b.slug, image: b.image, description: b.description })),
    elite: elite ? {
